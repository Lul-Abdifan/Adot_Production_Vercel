...
import { useGetTopicsQuery, useGetAllTopicsQuery } from "../../api/topic-api";
...
import { useEditInsightMutation } from "@/api/insight-api";
import { EditInsightFormData, EditInsightSchema } from "@/types/form-data";
...
export const EditInsightForm: FC<EditInsightFormProps> = ({ insight }) => {
  ...
  useEffect(() => {
    if (insight?.trimesters?.length) {
      const persistedTrimester = insight.trimesters
        .map((t: number) =>
          trimesterOptions.find((option) => option.value === t.toString())
        )
        .filter(Boolean); // Remove undefined values

      setSelectedTrimester(persistedTrimester as Option[]);
    }

    if (insight?.pregnancyWeeks?.length) {
      const persistedWeeks = insight.pregnancyWeeks.map((w: number) => ({
        label: `Week ${w}`,
        value: w.toString(),
      }));
      setSelectedWeeks(persistedWeeks);
    }
  }, [insight]);
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<EditInsightFormData>({
    mode: "onBlur",
    resolver: zodResolver(EditInsightSchema),
  });

  const showSuccessMessage = () => {
    toast.success("You have successfully updated an insight!", {
      position: toast.POSITION.TOP_RIGHT,
    });
  };

  const showErrorMessage = () => {
    toast.error("Insight update failed. Please try again.", {
      position: toast.POSITION.TOP_RIGHT,
    });
  };
  ...
  const [insightData, setInsightData] = useState<InsightFormData>({
    ...
    titleAmh: insight?.versions?.[1]?.title || "",
    descriptionAmh: insight?.versions?.[1]?.content || "",
    category: insight?.topic || "653a501efd4ad76e9b9e9b41",
    reference: ["Doctor"],
    reviewer: "654dce5811de6889bc7d5bc6",
    pregnancyWeeks: insight?.pregnancyWeeks,
    trimesters: insight?.trimesters,
  });
  ...
  const [updateInsight] = useEditInsightMutation();
  ...
  useEffect(() => {
    if (insightData.thumbnailImageFile) {
      ...
      setPreviewUrl(insight?.thumbnailImage);
    }
  }, [insightData.thumbnailImageFile, insight?.thumbnailImage]);
  ...
  const editInsight = async (data: EditInsightFormData) => {
    setLoading(true);

    if (!(insightData.thumbnailImageFile || previewUrl)) {
      setLoading(false);
      return;
    }
    ...
    if (insightData.thumbnailImageFile) {
      dataToSend.append("file", insightData.thumbnailImageFile);
    }
    ...
    const res: any = await updateInsight({
      data: dataToSend,
      id: insight._id,
    });
  ...
  return (
    <div className="flex w-full text-sm text-secondary flex-col items-center sm:justify-center">
      ...
      <div className="w-full rounded-md bg-white px-6 py6">
        <form onSubmit={handleSubmit(editInsight)}>
          ...
          <div className="flex mb-6 mt-2 gap-4">
                ...
                render={({ field }) => (
                  <MultiSelect
                    ...
                    onChange={(selected: Option[]) => {
                      ...
                      setSelectedWeeks([]); // Reset week selection when trimester changes
                      field.onChange(
            ...
            <div className="w-1/2">
              <label className="mb-2 block font-normal text-[15px]">
                Week of Pregnancy <span className="text-red-500">*</span>
              </label>
              <Controller
                control={control}
                name="pregnancyWeeks"
                ...
                render={({ field }) => (
                  <MultiSelect
                    ...
                    onChange={(selected: Option[]) => {
                      ...
                      field.onChange(selected.map((week) => week.value)); // Ensure values are updated
                    }}
          ...
          <div className="group grid grid-cols-2 gap-x-4 gap-y-8">
            <div>
              ...
              >
                Inisght title <span className="text-red-500">*</span>
              </label>
              <div className="flex flex-col items-start">
                <input
                  aria-hidden="true"
            ...
            <div className="row-span-2">
              ...
              <div className="flex flex-col items-start">
                <Controller
                  name="description"
                  control={control}
                  defaultValue={insightData.description}
                  render={({ field }) => (
                    <ReactQuill
                      className="h-80"
                      value={field.value}
            ...
            <div>
              ...
              <div className="flex flex-col items-start">
                <Controller
                  name="descriptionAmh"
                  control={control}
                  defaultValue={insightData.descriptionAmh}
                  render={({ field }) => (
                    <ReactQuill
                      className="h-80"
                      value={field.value}
            ...
            <div className="col-span-2">
              ...
              <section className="container w-full mx-auto items-center">
                <div className="pb-6 pt-4">
                  ...
                  >
                    <div className={`m-6 ${previewUrl ? "hidden" : ""} my-10`}>
                      <input
                        id="upload"
                        type="file"
                ...
                >
                  {topics
                    .filter((topic) =>
                      topic.versions.some(
                        (version) =>
                          version.language.toLowerCase() === "english"
                      )
                    )
                    .map((topic) => {
